...
callconf --prefix=/pkg/main/${PKG}.core.${PVRF} \
	...
	--enable-cuvid --enable-libnpp --enable-ffnvcodec --enable-vaapi --enable-cuda-nvcc --nvcc=/pkg/main/dev-util.nvidia-cuda-toolkit.core/bin/nvcc --nvccflags='-gencode arch=compute_80,code=sm_80 -O2 -I /pkg/main/dev-util.nvidia-cuda-toolkit.dev/include' --enable-libnpp --enable-nvenc
