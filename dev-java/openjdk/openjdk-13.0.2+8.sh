...
get http://hg.openjdk.java.net/jdk-updates/jdk13u/archive/jdk-"${PV}".tar.bz2

BOOT_JDK="/pkg/main/dev-java.openjdk.core"

if [ ! -d "$BOOT_JDK" ]; then
	# grab LFS bootjdk
	echo "Previous JDK not found, using one from LFS!"
	cd "${T}" || exit
...
# grab tests
cd "${CHPATH}/jdk13u-jdk-${PV}" || exit
get https://ci.adoptopenjdk.net/view/Dependencies/job/jtreg/lastSuccessfulBuild/artifact/jtreg-4.2-b16.tar.gz
